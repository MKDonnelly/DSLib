...
int __create_ll( void **list, int count, int element_size){
   ...
   (*head) = (mem_t*)malloc(element_size);
...
void *__find_node_ll( void **list, void *element, int (*compare_function)(void *each_in_list, void *reference_node) ){

    mem_t **head = (mem_t**)list;
    mem_t *stepper = *head;

    while( stepper != NULL ){
       if( compare_function( stepper, element ) != 0 )
          return stepper;
       stepper = stepper->next;
    }
    return 0;
